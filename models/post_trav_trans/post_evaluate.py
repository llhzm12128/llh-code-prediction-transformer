        ...
        if True:
            with torch.no_grad():
                ...
                for key in value_scores:
                    # print("{}".format(key))
                    value_ids = [a for a in batch["ids"][key] if a >= 0 and a < len(output)]    #叶子节点在AST中的位置（每个AST都是从零开始计算）
    
                    type_ids = [a for a in batch["ids"]["type_"+key] if a >= 0 and a < len(output)]#叶子的父类型节点在AST中的位置（每个AST都是从零开始计算） 
